...
public class Lista_sasiedztwa {
    ...
    void wypisz_Liste(){
        for(int i=0; i<m; i++){
            System.out.print(vertexArray.get(i).getValue() + "(" + vertexArray.get(i).getCondition() + "): ");
            for(int j=0; j<n; j++){
                if(edgeArray.get(j).getSource().getValue() == vertexArray.get(i).getValue()){
                    System.out.print(edgeArray.get(j).getDestination().getValue() + "(" + edgeArray.get(j).getDestination().getCondition() + ") ");
                }
            }
            System.out.println();
        }
